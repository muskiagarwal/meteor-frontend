    ...
    <section style={{ background: "#09090B", padding: "88px 28px" }}>
      <div style={{ maxWidth: 960, margin: "0 auto" }}>
        ...
        >
          ...
          <p style={{ fontSize: 16, color: "rgba(255,255,255,.4)" }}>Real outcomes from real job seekers.</p>
        </motion.div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 1, borderRadius: 12, overflow: "hidden", border: "1px solid rgba(255,255,255,.08)" }}>
          {STATS.map((s, i) => (
            <motion.div key={s.label} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1, duration: 0.5 }}
              style={{ padding: "40px 28px", background: "#111117", textAlign: "center", borderRight: i < 3 ? "1px solid rgba(255,255,255,.07)" : "none" }}
            >
              <div style={{ fontFamily: "var(--font-bricolage)", fontWeight: 800, fontSize: 48, letterSpacing: "-.04em", marginBottom: 8, background: "linear-gradient(135deg,#C5F135,#a8cc20)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundClip: "text" }}>
